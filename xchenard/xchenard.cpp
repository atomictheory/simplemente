...
bool IsInputReady()
{
    ...
    fd_set fd;
    timeval noblock = {0};      // zero-valued timeout will prevent select() from blocking
...
void MakeMove (const char *original, Move move)
{
    UnmoveInfo unmove;
...
void ReceiveMove (const char *original, int source, int dest, int prom)
{
    if (BoardIsCorrupt) {
        ...
        if (found == 1) {
            MakeMove (original, ml.m[matchIndex]);
        } else {
            SendMoveRejection (original);
        }
    }
...
ponder_again:
                ...
                printf ("move %s\n", moveString);
                MakeMove (moveString, move);    // must do this *AFTER* transmitting 'move' command, in case it is end of game!
...
bool CommandMayBeIgnored (const char *verb, const char *rest)
{
...
bool ExecuteCommand (const char *verb, const char *rest)        // returns true if 'quit' received
{
    ...
    if (CommandMayBeIgnored (verb, rest)) {
    ...
    if (0 == strcmp(verb,"quit")) {
		...
		if((strcmp(rest,"standard")==0)||(strcmp(rest,"normal")==0))
		{
			// variant atomic
			xboard_variant=VARIANT_STANDARD;
		}
		else if(strcmp(rest,"atomic")==0)
		{
			// variant atomic
			xboard_variant=VARIANT_ATOMIC;
		}
		else
		{
